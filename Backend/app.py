from datetime import datetime #Give the option work on date and time
from flask import Flask, jsonify, request
from flask_jwt_extended import JWTManager, create_access_token, get_jwt_identity, jwt_required
from flask_bcrypt import Bcrypt #extension for password hashing
from flask_cors import CORS #extension for handling CORS
...
jwt = JWTManager(app)
bcrypt = Bcrypt(app) #Configures the Flask,JWTManager,and Bcrypt

# Set up SQLite database
engine = create_engine('sqlite:///db.sqlite3', echo=False) 
Session = sessionmaker(bind=engine) #the connection between what the customer write on INDEX into the DATA-BASE
Base.metadata.create_all(bind=engine) #creates the database tables defined in the models by using the specified engine
...
#for removing a customer account 
...
#for user login, generates a JWT token upon successful login.
...
#for getting all books or adding a new book 
...
#for getting, updating, or deleting a specific book 
...
# for getting customer details by ID
...
# for searching customers by name
...
#for getting all loans or adding a new loan 
...
#for getting or returning a specific loan 
